...
impl Default for IconTheme {
    fn default() -> Self {
        IconTheme {
            name: ByFilename::default_name(),
            extension: ByFilename::default_extension(),
...
fn deserialize_by_name<'de, D>(deserializer: D) -> Result<ByFilename, D::Error>
where
    D: serde::de::Deserializer<'de>,
{
    let default = ByFilename::default_name().hash;
    HashMap::deserialize(deserializer).map(|input| ByFilename {
        hash: merge_hashmap(default, input),
    })
}

fn deserialize_by_extension<'de, D>(deserializer: D) -> Result<ByFilename, D::Error>
where
    D: serde::de::Deserializer<'de>,
{
    let default = ByFilename::default_extension().hash;
    HashMap::deserialize(deserializer).map(|input| ByFilename {
        hash: merge_hashmap(default, input),
    })
}
...
impl IntoIterator for ByFilename {
